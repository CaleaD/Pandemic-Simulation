...
diagram3  = function(file="SIR + Vaccination.png", save.png=FALSE,scaleX=1/2, scaleY=1/2) {
  ...
  # Inf young
  coord[4,1] = 0.5
  coord[4,2] = 0.6 - 0.2 * scaleY
  
  # Inf old
  coord[5,1] = 0.5
  coord[5,2] = 0.3 + 0.2 * scaleY
  ...
  curvedarrow(from = c(0.5, 0.5 + 0.02 * scaleY), to = c(0.5 - 0.2*scaleX, 0.5 + 0.11*scaleY), lcol = "red",
              curve =0.7, arr.pos = 0.95)
  
  # from Inf old to SusOld
  curvedarrow(from = c(0.5, 0.5 -0.25 * scaleY), to = c(0.5 - 0.2*scaleX, 0.5 - 0.22 *scaleY), lcol = "red",
              curve =-0.7, arr.pos = 0.95)
  
  # from H to Sy
  curvedarrow(from = c(0.5 + 0.17*scaleX, 0.5 + 0.4 * scaleY), to = c(0.5 - 0.1*scaleX, 0.5 + 0.22*scaleY), lcol = "orange",
              curve = 0.2, arr.pos = 0.9)
  
  # from H to So
  curvedarrow(from = c(0.5 + 0.21*scaleX, 0.5 +0.37*scaleY), to = c(0.5 -0.1*scaleX, 0.5 - 0.38*scaleY), lcol = "orange",
              curve = -0.2, arr.pos = 0.9)     
