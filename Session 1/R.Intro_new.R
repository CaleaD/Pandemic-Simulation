...
sir <- function(time, state, parameters) {
  with(as.list(c(state, parameters)), {
    dS = young*(-infect * S * I - infect * S * H); # both I & H infect!
    dV = old*(-infect * V * I - infect * V * H)
    dI =  young*(infect * S * I + infect * S * H) + old*(infect * V * I + infect * V * H) - recov * I - death * I - hosp * I;
    dH =  hosp * I - recov.h * H - death.h * H - recov.h * V - death.h * V;
    dR =  recov * I + recov.h * H + recov * V + recov.h * V; # done
    dD =  death * I + death.h * H + death * V + death.h * V; # done
    
    return(list(c(dS,dV, dI, dR, dD, dH)));
  })
}
...
parameters = c(infect = infect,
               recov = recov[1], recov.h = recov[2],
               death=death[1], death.h = death[2], hosp = hosp,old = old,young=young)
init = c(S = 1-1e-6, I = 1e-6, R = 0.0, D = 0.0, H = 0.0)
...
### Plot
plot.sir(out, times, legend.lbl = c("Young", "Old", "Infected","Recovered","Death", "Hosp"), leg.off=c(0, -0.2))
...
sir <- function(time, state, parameters) {
  with(as.list(c(state, parameters)), {
	dV = min(S, vacc); # V protects, but reduces S;
